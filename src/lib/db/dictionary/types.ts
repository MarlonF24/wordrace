...
export const SELECTABLE_SENSE_LEXICAL_KEYS = [...SELECTABLE_SHARED_LEXICAL_KEYS, ...SELECTABLE_EXCLUSIVE_SENSE_LEXICAL_KEYS] as const;
export type SelectableSenseLexicalKey = typeof SELECTABLE_SENSE_LEXICAL_KEYS[number];


export const SELECTABLE_ENTRY_LEXICAL_KEYS = [...SELECTABLE_SHARED_LEXICAL_KEYS, ...SELECTABLE_EXCLUSIVE_ENTRY_LEXICAL_KEYS] as const;
export type SelectableEntryLexicalKey = typeof SELECTABLE_ENTRY_LEXICAL_KEYS[number];

export const SELECTABLE_LEXICAL_KEYS = [...SELECTABLE_SHARED_LEXICAL_KEYS, ...SELECTABLE_EXCLUSIVE_SENSE_LEXICAL_KEYS, ...SELECTABLE_EXCLUSIVE_ENTRY_LEXICAL_KEYS] as const;
...
export const OBJECT_FIELDS_TO_PRINT: {
  [K in keyof FlatObjectSelectableLexicalFields]: readonly (KeysOfValue<FlatObjectSelectableLexicalFields[K], string>)[]
} = {
  antonyms: ["word"],
  synonyms: ["word"],
  hypernyms: ["word"],
...
type WithRichText<T> = T extends string 
  ...
  export type ProcessedlexicalField<K extends SelectableLexicalKey> = 
  (K extends FlatObjectSelectableLexicalKey // if linking to object or object[]
    ? RawLexicalFields[K] extends unknown[] // if linking to object[]
      ? ProcessedFlatObjectlexicalField<K>[]
      : ProcessedFlatObjectlexicalField<K>
      : K extends "glosses" ? RichText // glosses are an exception where after processing the last element of the string[] becomes a RichText
...
export type SelectableLexicalFields = Pick<EntryLexicalFields, SelectableEntryLexicalKey> & Pick<SelectableSenseLexicalFields, SelectableExclusiveSenseLexicalKey>;

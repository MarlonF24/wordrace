...
- `RichToken`: `{ w, l, p }`, where `w` is the surface word, `l` is the lemma, and `p` is a Wiktionary-style POS tag.
...
`getWordRecord` fetches a processed record by lowercased word and filters it to the selected lexical fields for a game. It throws when the word does not exist or when the requested fields contain nothing displayable for that word.

Filtering happens on already-processed JSONB. It does not mutate the stored dictionary rows.

## Search Contract

The generated `allLinks` field is for graph/search consumers. It is intentionally based on `w` and `l` link data. The `p` field is rendering metadata and should not be required by `ML_agent`.

...
import { isWinkPosTag } from './part-of-speech';
 ...
 * layout. Word-like tokens carry surface form, lemma, and Wink POS metadata for
 * game movement and search filtering.
 */
export function tokenizeToRichText(text: string): RichText {
    ...
    for (let idx = 0; idx < tokens.length(); idx++) {
        ...
        if (!isWinkPosTag(pos)) {
            throw new Error(`Unknown wink-nlp POS tag: ${pos}`);
        }
